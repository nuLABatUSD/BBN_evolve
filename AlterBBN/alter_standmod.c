...
{ 
	...
	else 
	{
		...
		if(argc>5) sscanf(argv[5],"%d",&coupD); else coupD=1;		
		if(argc>6) sscanf(argv[6],"%lf",&Tdrh); else Tdrh=0.;
     		if(argc>7) sscanf(argv[7],"%lf",&Tsrh); else Tsrh=0.;   
     		if(argc>8) sscanf(argv[8],"%d",&failsafe); else failsafe=1;   
  	}
	...
	Init_dark_density(dd0,ndd,Tdrh*1.e-3,&paramrelic);
